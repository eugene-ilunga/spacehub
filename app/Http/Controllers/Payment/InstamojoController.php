...
class InstamojoController extends Controller
{
    ...
    {
        try {
            $data = OnlineGateway::whereKeyword('instamojo')->firstOrFail();
            $paydata = $data->convertAutoData();

            $api = $paydata['sandbox_status'] == 1
                ? new Instamojo($paydata['key'], $paydata['token'], 'https://test.instamojo.com/api/1.1/')
                : new Instamojo($paydata['key'], $paydata['token']);
            ...
            return redirect()->route($_cancel_url)->with('error', 'Payment failed: ' . $e->getMessage());
        }
    }
