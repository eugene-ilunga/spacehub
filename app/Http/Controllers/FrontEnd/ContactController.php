...
class ContactController extends Controller
{
  public function contact()
  {
    ...
    $queryResult['seoInfo'] = $language->seoInfo()->select('meta_keyword_contact as meta_keywords', 'meta_description_contact as meta_description')->first();

    $queryResult['pageHeading'] = $misc->getPageHeading($language);

    $queryResult['breadcrumb'] = $misc->getBreadcrumb();

    $queryResult['contactContent'] = $language->contactContent()->first();
    $queryResult['contact'] = Contact::first();

    $queryResult['emails'] = explode(",", $queryResult['contact']?->email_address);
    $queryResult['mobiles'] = explode(",", $queryResult['contact']?->mobile_number);
  ...
  public function sendMail(MailFromUserRequest $request)
  {
    ...
    $messageText = '<p>' . __('A new quote request has been sent') . '.<br/><strong>' . __('Client Name') . ': </strong>' . $name . '<br/><strong>' . __('Client Mail') . ': </strong>' . $request->email . '</p><p>' . __('Message') . ': ' . nl2br(Purifier::clean($request->message, 'youtube')) . '</p>';
    ...
    try {
      ...
      Session::flash('success',__('Mail has been sent') . '.');
    } catch (Throwable $e) {
