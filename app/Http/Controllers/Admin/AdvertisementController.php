...
class AdvertisementController extends Controller
{
  ...
  public function updateAdvertiseSettings(Request $request)
  {
    ...
    $request->session()->flash('success', __('Advertise settings updated successfully') . '!');
  ...
  {
    $ads = Advertisement::query()->orderByDesc('id')->paginate(10);
  ...
  public function store(StoreRequest $request)
  {
    ...
    Advertisement::query()->create($request->except('image') + [
      'image' => $request->hasFile('image') ? $imageName : null
    ]);

    $request->session()->flash('success', __('New advertisement added successfully') . '!');
  ...
  {
    $ad = Advertisement::query()->find($request->id);

    if ($request->hasFile('image')) {
      $newImg = $request->file('image');
      $oldImg = $ad->image;
      $imageName = UploadFile::update('./assets/img/advertisements/', $newImg, $oldImg);
    }
    ...
    $request->session()->flash('success', __('Advertisement updated successfully') . '!');
  ...
  public function bulkDestroy(Request $request)
  {
    ...
    $request->session()->flash('success', __('Advertisements deleted successfully') . '!');
