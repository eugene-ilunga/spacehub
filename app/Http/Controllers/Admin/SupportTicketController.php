...
class SupportTicketController extends Controller
{
      ...
      'attachment' => [
        function ($attribute, $value, $fail) use ($fileExtension) {
          if (strcmp('zip', $fileExtension) != 0) {
            $fail(__('The') . ' ' . $attribute . ' '. __('must be a file of type') . ': zip') . '.';
          }
        },
        'max:' . $maxSize
      ]
  ...
  public function destroy($id)
  {
    ...
    return redirect()->back()->with('success', __('Ticket has been deleted') . '!');
  }
  ...
  public function bulkDestroy(Request $request)
  {
    ...
    $request->session()->flash('success', __('Tickets have been deleted') . '!');
