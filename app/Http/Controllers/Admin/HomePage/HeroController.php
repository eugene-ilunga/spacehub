...
class HeroController extends Controller
{
  public function index(Request $request)
  {
    ...
    if ($themeVersion == 1 || $themeVersion == 2 || $themeVersion == 3 ){
      $information['heroImgs'] = Basic::query()->select('hero_bg_img', 'hero_static_img', 'hero_static_img_1')->first();
      $information['heroInfo'] = $language->heroStatic()->first();
      return view('admin.home-page.hero-section.static.index', $information);
    } 
    else {
  ...
  public function updateImg(Request $request)
  {
    ...
    $bgImg = Basic::query()->select('hero_bg_img', 'hero_static_img','hero_static_img_1', 'theme_version')->first();

    $rules = [];
    if($bgImg->theme_version === 3)
    {
      if (empty($bgImg->hero_static_img_1)) {
        $rules['hero_static_img_1'] = 'required';
      }
    }
    ...
    $message = [
      ...
      'hero_static_img_1.required' => 'The Hero Static image field is required.'
    ];
    ...
    if ($request->hasFile('hero_static_img_1')) {
      $newImage3 = $request->file('hero_static_img_1');
      $oldImage3 = $bgImg->hero_static_img_1;
      $imgName3 = UploadFile::update('./assets/img/', $newImage3, $oldImage3);
    } else {
      $imgName3 = $bgImg->hero_static_img_1;
    }
    $basic = Basic::where('uniqid', 12345)->first();
    $basic->hero_bg_img = $imgName;
    $basic->hero_static_img = $imgName2;
    $basic->hero_static_img_1 = $imgName3;
