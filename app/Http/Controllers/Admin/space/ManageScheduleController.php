...
class ManageScheduleController extends Controller
{
  public function index(Request $request)
  {
    ...
    $data['days'] = GlobalDay::where('space_id', $request->space_id)->paginate(10);
    $data['space_id'] = $request->space_id;
    return view('admin.manage-schedule.time-slot.index', $data);
  }
  public function manageWeekend(Request $request)
  {
    ...
    $data['days'] = GlobalDay::where('space_id', $request->space_id)->paginate(10);
    $data['space_id'] = $request->space_id;
    return view('admin.manage-schedule.time-slot.weekend', $data);
  }
  ...
  public function manageSchedule(Request $request)
  {
    ...
    $defaultLanguageId = getAdminLanguage()->id;
    ...
    return view('admin.manage-schedule.time-slot.manage-time-slot', $data);
  }
  ...
  public function storeTimeSlot(StoreTimeSlotRequest $request)
  {
    ...
    $inputStartTime = trim($request->input('start_time')); // e.g., 06:00 or 6:00 AM
    $inputEndTime = trim($request->input('end_time'));     // e.g., 13:00 or 1:00 PM
  ...
  public function updateHoliday(Request $request, $id)
  {
    ...
    $days = GlobalDay::query()->select('is_holiday', 'id')->findOrFail($id);
    $days->update([
  ...
  public function update(UpdateTimeSlotRequest $request)
  {
    ...
    $timezone = now()->timezoneName; // Get admin's timezone dynamically
