                        ...
                        <div class="col-lg-3">
                            <form action="" method="get">
                                ...
                                <input type="text" value="{{ request()->input('transaction_id') }}" name="transaction_id"
                                    placeholder="{{ __('Enter transaction id') }}" class="form-control w-100">
                            </form>
                        </div>
                    </div>
                </div>
                                        ...
                                        <tbody>
                                            @foreach ($transactions as $transaction)
                                                <tr>
                                                    ...
                                                    <td>
                                                        @php
                                                            $seller = $transaction->seller()->first();
                                                        @endphp
                                                        @if ($seller)
                                                            <a
                                                                href="{{ route('admin.end-user.vendor.details', ['id' => $seller->id, 'language' => $defaultLang->code]) }}">{{ $seller->username }}</a>
                                                        @else
                                                    ...
                                                    <td>
                                                        ...
                                                        @elseif ($transaction->transcation_type == 6)
                                                            {{ __('Space Feature') }}
                                                        @elseif ($transaction->transcation_type == 7)
                                                            {{ __('Product Purchase') }}
                                                        @endif
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td class="ltr">
                                                        @if ($transaction->transcation_type == 2 || $transaction->transcation_type == 4)
                                                    ...
                                                    <td>
                                                        ...
                                                        @elseif ($transaction->transcation_type == 7)
                                                                @php
                                                                    $productOrdered = \App\Models\Shop\ProductOrder::where('order_number', $transaction->booking_number)->select('invoice')->first();
                                                                @endphp
                                                                    @if ($productOrdered)
                                                                    <a target="_blank" class="btn btn-secondary btn-sm mr-1" href="{{ asset('assets/file/invoices/product/'. $productOrdered->invoice) }}"> <i class="fas fa-eye"></i></a>
                                                                @endif
                                                        @endif
                                                    </td>
                                                </tr>
