...
@section('content')
          ...
          <div class="payment-information">
            ...
            @if ($orderInfo->booking_type == 1 || $orderInfo->booking_type == 2)
              <div class="row mb-2">
                ...
                <div class="col-lg-9"><span dir="ltr">{{ $orderInfo->start_time }}</span></div>
              </div>
              @if ($orderInfo->booking_type == 1)
                <div class="row mb-2">
                  ...
                  <div class="col-lg-9"><span dir="ltr">{{ $orderInfo->end_time }}</span></div>
                </div>
              @endif
              @if ($orderInfo->booking_type == 2)
                <div class="row mb-2">
                  ...
                  <div class="col-lg-9"><span dir="ltr">{{ $orderInfo->end_time_without_interval }}</span></div>
                </div>
              @endif
            @endif
            ...
            <div class="row mb-2">
              ...
              <div class="col-lg-9">
                @if (!empty($orderInfo->space_slug))
                  <a target="_blank"
                    href="{{ route('space.details', ['slug' => $orderInfo->space_slug, 'id' => $orderInfo->space_id]) }}" target="_blank">
            ...
            @if (!empty($orderInfo->number_of_guest))
              <div class="row mb-2">

                <div class="col-lg-3">
                  @if ($orderInfo->number_of_guest > 1)
            ...
            @endif
            @if (!is_null($orderInfo->sub_total))
              <div class="row mb-2">
                <div class="col-lg-3 ltr">
                ...
                <div class="col-lg-9 ltr">
                  ...
                  {{ $orderInfo->seller_id != null ? '(' . __('Received by vendor') . ')' : '' }}
                </div>
              </div>
            @endif
            ...
            <div class="row mb-2">
              <div class="col-lg-3">
                <strong>{{ __('Booked By') . ' :' }}</strong>
              </div>

              <div class="col-lg-9">
                @if (is_null($orderInfo->booked_by))
                  {{ __('Website') }}
                @else
                  {{ $orderInfo->booked_by == 'admin' ? __('admin') : __('Vendor') }}
                @endif
              </div>
            </div>
            <div class="row mb-2">
              ...
              @if ($orderInfo->booked_by === 'admin' || $orderInfo->booked_by === 'vendor')
                <div class="col-lg-3">
                  <select name="payment_method" class="form-control">
                    @foreach ($gateways as $gateway)
                      @if ($gateway->type == 'online')
                        <option value="{{ $gateway->name }}"
                          {{ $orderInfo->payment_method == $gateway->keyword ? 'selected' : '' }}>
                          {{ __($gateway->name) }}
                        </option>
                      @endif
                    @endforeach
                    @foreach ($gateways as $gateway)
                      @if ($gateway->type == 'offline')
                        <option value="{{ $gateway->name }}"
                          {{ $orderInfo->payment_method == $gateway->name ? 'selected' : '' }}>
                          {{ __($gateway->name) }}
                        </option>
                      @endif
                    @endforeach
            ...
            <div class="row mb-2">
              ...
              <div class="col-lg-9">
                @if (!empty($orderInfo->booked_by))
                  @if (!is_null($orderInfo->grand_total))
                    <form id="paymentStatusForm" class="d-inline-block"
                      action="{{ route('admin.booking_record.update_payment_status', ['id' => $orderInfo->id]) }}"
                      method="post">
                      @csrf
                      <select
            ...
            <div class="row mb-1">
              ...
              <div class="col-lg-9">
                <span
                  class="badge @if ($orderInfo->booking_status == 'pending') badge-primary @elseif ($orderInfo->booking_status == 'approved') badge-success @elseif ($orderInfo->booking_status == 'rejected') badge-danger @endif">
                  @if ($orderInfo->booking_status == 'pending')
                    {{ __('Pending') }}
                  @elseif($orderInfo->booking_status == 'approved')
                    {{ __('Approved') }}
                  @elseif($orderInfo->booking_status == 'rejected')
                    {{ __('Rejected') }}
                  @else
                    {{ __('Pending') }}
                  @endif
          ...
          <div class="payment-information">
            @if (!empty($orderInfo->customer_name))
              <div class="row mb-2">
                ...
                <div class="col-lg-9">
                  <a
                    href="{{ route('admin.user_management.user.details', ['id' => $orderInfo->user_id, 'language' => $defaultLang->code]) }}">
            ...
            <div class="payment-information">
              ...
              @if (!empty($sellerInfo->username))
                <div class="row mb-2">
                  ...
                  <div class="col-lg-9">
                    ...
                    <a
                      href="{{ route('admin.end-user.vendor.details', ['id' => $orderInfo->seller_id, 'language' => $defaultLang->code]) }}">
                      {{ $sellerInfo->username ?? '' }}
                    </a>
                  </div>
                </div>
              ...
              @if (!empty($sellerInfo->email))
                <div class="row mb-2">
                  ...
                  <div class="col-lg-9">{{ $sellerInfo->email ?? '' }}</div>
                </div>
              @endif
              @if (!empty($sellerInfo->phone))
                <div class="row mb-2">
                  ...
                  <div class="col-lg-9">{{ $sellerInfo->phone ?? '' }}</div>
                </div>
              @endif
              @if (!empty($sellerInfo->city))
                <div class="row mb-2">
                  ...
                  <div class="col-lg-9">{{ $sellerInfo->city ?? '' }}</div>
                </div>
              @endif
              @if (!empty($sellerInfo->state))
                <div class="row mb-2">
                  <div class="col-lg-3">
                    <strong>{{ __('State') . ' :' }}</strong>
                  </div>
                  <div class="col-lg-9">{{ $sellerInfo->state ?? '' }}</div>
                </div>
              @endif
              @if (!empty($sellerInfo->zip_code))
                <div class="row mb-2">
                  <div class="col-lg-3">
                    <strong>{{ __('Zip code') . ' :' }}</strong>
                  </div>
                  <div class="col-lg-9">{{ $sellerInfo->zip_code ?? '' }}</div>
                </div>
              @endif
              @if (!empty($sellerInfo->address))
                <div class="row mb-2">
                  ...
                  <div class="col-lg-9">{{ $sellerInfo->address ?? '' }}</div>
                </div>
              @endif
              @if (!empty($sellerInfo->country))
                <div class="row mb-2">
                  ...
                  <div class="col-lg-9">{{ $sellerInfo->country ?? '' }}</div>
                </div>
              @endif
            </div>
          </div>
  ...
  @include('admin.booking-management.service-details')
@endsection
