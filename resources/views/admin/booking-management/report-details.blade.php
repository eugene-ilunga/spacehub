              ...
              <div class="table-responsive">
                <table class="table table-striped mt-3" id="basic-datatables">
                  <thead>
                    <tr>
                      ...
                      <th scope="col">{{ __('Price') }}
                        ({{ $basic->base_currency_text_position == 'left' ? $basic->base_currency_text . ' ' :
                        ($basic->base_currency_text_position == 'right' ? ' ' . $basic->base_currency_text : '') }})
                      </th>
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td>
                        {{ strlen($service->service_title) > 50 ? mb_substr($service->service_title, 0, 50, 'UTF-8') .
                      ...
                      <td>
                        ...
                        <div>
                          <strong>{{__('Title')}} </strong>: <span class="ml-2">{{@$subservice->sub_service_title
                            }}</span>
                          <br>
                          <strong>{{__('Price')}} </strong>: <span class="ml-2"> {{ $basic->base_currency_symbol_position
                            == 'left' ?
                            $basic->base_currency_symbol : '' }}{{@$subservice->sub_service_price }} {{
                            $basic->base_currency_symbol_position == 'right' ?
                            $basic->base_currency_symbol : '' }}</span>
                        </div>
                        <br>
                        @endforeach
                      </td>
