...
@section('content')
    <div class="page-header">
        ...
        <ul class="breadcrumbs">
            ...
            <li class="nav-item">
                <a href="{{ route('admin.end-user.vendor.registered_vendor', ['language' => $defaultLang->code]) }}">{{ __('Registered vendors') }}</a>
            </li>
            <li class="separator">
                <i class="flaticon-right-arrow"></i>
            </li>
                    ...
                    <div class="row">
                        <div class="col-lg-6 offset-lg-3">
                            <form id="ajaxEditForm"
                                  action="{{ route('admin.end-user.vendor.update_password', ['id' => $sellerInfo->id]) }}"
                                  method="post">
                                @csrf
                                <div class="form-group">
                                    <label>{{ __('New Password') . '*' }}</label>
                                    <input type="password" class="form-control" name="new_password" placeholder="{{ __('Enter new password') }}">
                                    <p id="editErr_new_password" class="mt-1 mb-0 text-danger em"></p>
                                </div>

                                <div class="form-group">
                                    <label>{{ __('Confirm New Password') . '*' }}</label>
                                    <input type="password" class="form-control" name="new_password_confirmation" placeholder="{{ __('Enter confirm new password') }}">
