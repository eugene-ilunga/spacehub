    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLongTitle">{{ __('Edit Space Subcategory') }}</h5>
            ...
            <div class="modal-body">
                <form id="ajaxEditForm" class="modal-form"
                    action="{{ route('admin.space_management.sub-category.update') }}" method="post">
                    @csrf
                    <input type="hidden" id="in_id" name="id">

                    <div class="form-group">
                        <label for="">{{ __('Category') . '*' }}</label>
                        <select name="space_category_id" class="form-control" id="in_space_category_id">
                            <option disabled>{{ __('Select a Category') }}</option>
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}">{{ $category->name }}</option>
                            @endforeach
                        </select>
                        <p id="editErr_space_category_id" class="mt-2 mb-0 text-danger em"></p>
                    </div>
