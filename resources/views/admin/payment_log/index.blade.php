      ...
      <div class="card-header">
        <div class="row align-items-center">
          ...
          <div class="col-lg-2 col-12 mb-3 mb-lg-0">
            <h4 class="card-title mb-0">{{ __('Subscription Log') }}</h4>
          </div>

          <!-- Filters Column (Right Side) -->
          <div class="col-lg-7 col-12 ">
            <form id="filterForm" action="{{ url()->current() }}" method="GET" class="row">
              <input type="hidden" name="language" value="{{ $defaultLang->code }}">

              <!-- Username Search -->
              <div class="col-lg-4 col-md-4 col-12 mb-2 mb-lg-0 px-1">
                <input type="text" name="username" class="form-control w-100" placeholder="{{ __('Username') }}"
                  value="{{ request('username') }}" onkeypress="if(event.keyCode == 13) this.form.submit()">
              </div>

              <!-- Transaction ID Search -->
              <div class="col-lg-4 col-md-4 col-12 mb-2 mb-lg-0 px-1">
                <input type="text" name="search" class="form-control w-100" placeholder="{{ __('Transaction ID') }}"
                  value="{{ request('search') }}" onkeypress="if(event.keyCode == 13) this.form.submit()">
              </div>

              <!-- Payment Status -->
              <div class="col-lg-4 col-md-4 col-12 mb-2 mb-lg-0 px-1">
                <select class="form-control w-100" name="payment_status" onchange="this.form.submit()">
                  <option value="">{{ __('All Status') }}</option>
                  <option value="completed" @selected(request('payment_status')=='completed' )>{{ __('Completed') }}
                  </option>
                  <option value="pending" @selected(request('payment_status')=='pending' )>{{ __('Pending') }}</option>
                  <option value="rejected" @selected(request('payment_status')=='rejected' )>{{ __('Rejected') }}
                  </option>
                </select>
              </div>
            </form>
            ...
            <div class="table-responsive">
              <table class="table table-striped mt-3">
                ...
                <tbody>
                  ...
                  <tr>
                    <td>
                      {{ strlen($membership->transaction_id) > 30 ? mb_substr($membership->transaction_id, 0, 30,
                      'UTF-8') . '...' : $membership->transaction_id }}
                    </td>
                    <td>
                      @php
                      $seller = $membership->seller()->first();
                      @endphp
                      @if ($seller)
                      <a target="_blank"
                        href="{{ route('admin.end-user.vendor.details', ['id' => $seller->id, 'language' => $defaultLang->code]) }}">{{
                        $seller->username }}</a>
                      @endif
                    </td>
                    @php
                    ...
                    <td>
                      @if (json_decode($membership->transaction_details) !== 'offline' || $membership->status == 1 ||
                      $membership->status == 2)
                      ...
                      <form id="statusForm{{ $membership->id }}" class="d-inline-block"
                        ...
                        <input type="hidden" name="payment_method" value="{{ $membership->id }}">
                        <select class="form-control form-control-sm
                          ...
                          <option value=0 {{ $membership->status == 0 ? 'selected' : '' }}>
                            {{ __('Pending') }}
                          </option>
                          <option value=1 {{ $membership->status == 1 ? 'selected' : '' }}>
                            {{ __('Success') }}
                          </option>
                          <option value=2 {{ $membership->status == 2 ? 'selected' : '' }}>
                            {{ __('Rejected') }}
                          </option>
                        </select>
                      </form>
                    ...
                    <div class="modal-dialog" role="document">
                      <div class="modal-content">
                        ...
                        <div class="modal-body">
                          @if (!empty($membership->receipt))
                          <img src="{{ asset('assets/frontend/images/membership/receipt/' . $membership->receipt) }}"
                    ...
                    <div class="modal-dialog" role="document">
                      <div class="modal-content">
                        ...
                        <div class="modal-body">
                          ...
                          <p>{{ !empty($membership->seller) ? $membership->seller->username : '-' }}
                          </p>

                          <label>{{ __('Email') }}</label>
                          <p>{{ !empty($membership->seller) ? $membership->seller->email : '-' }}
                          </p>
                          <label>{{ __('Phone') }}</label>
                          <p>
                            {{ !empty($membership->seller->phone) ? $membership->seller->phone : '-' }}
                          </p>
                          <h3 class="text-warning">{{ __('Payment details') }}</h3>
                          <p><strong>{{ __('Package Price') }}
                              : </strong> {{ $membership->price }}
                          </p>
                          @if ($membership->discount > 0)
                          <p><strong>{{ __('Discount') }}
                              : </strong> {{ $membership->discount }}
                          </p>
                          <p><strong>{{ __('Total') }}
                              : </strong> {{ $membership->price }}
                          </p>
                          @endif
                          <p><strong>{{ __('Currency') }}
                              : </strong> {{ $membership->currency }}
                          </p>
                          <p><strong>{{ __('Method') }}
                              : </strong> {{ __($membership->payment_method) }}
                          </p>
                          <h3 class="text-warning">{{ __('Package Details') }}</h3>
                          <p><strong>{{ __('Title') }}:
                            </strong>{{ !empty($membership->package) ? __($membership->package->title) : '' }}
                          </p>
                          ...
                          <p><strong>{{ __('Expire Date') }}: </strong>
                            ...
                            @else
                            {{ $membership->package->term == 'lifetime' ? __('Lifetime') :
                            \Illuminate\Support\Carbon::parse($membership->expire_date)->format('M-d-Y') }}
                            @endif
                            @endif
                          </p>
                          <p>
