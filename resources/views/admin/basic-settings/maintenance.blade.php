                    ...
                    <div class="row">
                        <div class="col-lg-6 offset-lg-3">
                            <form id="maintenanceForm"
                                  action="{{ route('admin.basic_settings.update_maintenance_mode') }}"
                                ...
                                <div class="form-group">
                                    ...
                                    <p class="text-warning small mt-2">{{ '*' . __('Recommended Image Size') . ':' }} <strong dir="ltr">775×720 px</strong></p>
                                        ...
                                        <label class="selectgroup-item">
                                            <input type="radio" name="maintenance_status" value="1"
                                                   class="selectgroup-input"
                                                    {{ !empty($data) && $data->maintenance_status == 1 ? 'checked' : '' }}>
                                            <span class="selectgroup-button">{{ __('Active') }}</span>
                                        </label>
                                        <label class="selectgroup-item">
                                            <input type="radio" name="maintenance_status" value="0"
                                                   class="selectgroup-input"
                                                    {{ !empty($data) && $data->maintenance_status == 0 ? 'checked' : '' }}>
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control" name="maintenance_msg" rows="3"
                                              cols="80">{{ !empty($data) ? $data->maintenance_msg : '' }}</textarea>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" name="bypass_token"
                                           value="{{ !empty($data) ? $data->bypass_token : '' }}">
                                    <p class="mt-2 mb-0 text-warning">
                                        {{ '*' .__('During maintenance'). ', '. __('you can access the system by using this token') . '.' }}
                                        <br>
                                        <span class="text-info">{{ url('/') . '/' . __('token-value')}}</span><br>
                                        {{ '*' . __('Do not use special character in this token') . '.' }}
                                    </p>
                                </div>
                            </form>
                        </div>
