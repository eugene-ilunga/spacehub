...
@endsection
@php
    $title = $pageHeading->customer_order_details_page_title ?? __('Order Details');
@endphp
@section('content')
    <!-- Breadcrumb start -->
                        ...
                        <div class="order-details radius-md">
                            @if ($order->order_status != 'rejected')
                                <div class="progress-area-step">
                                    <ul class="progress-steps d-flex justify-content-center">
                                        ...
                                        @endphp
                                        <li
                                            class="{{ in_array($order->order_status, ['pending', 'processing', 'completed']) ? 'active' : '' }}">
                                            <div class="icon">1</div>
                                            <div class="progress-title" style="color: {{ $order_status_color }}">
                                                {{ __('Order placed') }}</div>
                                        </li>
                                        <li
                                            class="{{ in_array($order->order_status, ['processing', 'completed']) ? 'active' : '' }}">
                                        ...
                                        <li
                                            class="{{ in_array($order->order_status, ['completed']) ? 'active' : '' }}">
                                    ...
                                    <div class="row align-items-center">
                                        <div class="col-lg-8">
                                            ...
                                            <div class="order-info mb-20">
                                                <h6>{{ __('Order List') . ': ' . '#' }}{{ $order->order_number }} <span
                                                        class="{{ $text_color }}">[{{ __(ucfirst($order->order_status) )}}]</span>
                                                </h6>
                                ...
                                <div class="row">
                                    <div class="col-xxl-4 col-lg-6 col-md-6">
                                        <div class="main-info mb-30">
                                            <h5 class="mb-10">{{ __('Billing Address') }}</h5>
                                            <ul class="list">
                                                ...
                                                @if (!empty($order->billing_state))
                                                    <li><span>{{ __('State') . ':' }}</span>{{ $order->billing_state }}
                                                    </li>
                                                @endif
                                                <li><span>{{ __('Country') . ':' }}</span> {{ $order->billing_country }}
                                                </li>
                                                <li><span>{{ __('Address') . ':' }}</span> {{ $order->billing_address }}
                                                </li>
                                            </ul>
                                        </div>
                                    </div>
                                    <div class="col-xxl-4 col-lg-6 col-md-6">
                                        <div class="main-info mb-30">
                                            <h5 class="mb-10">{{ __('Shipping Address') }}</h5>
                                            <ul class="list">
                                                ...
                                                @if (!empty($order->shipping_state))
                                                    <li><span>{{ __('State') . ':' }}</span>{{ $order->shipping_state }}
                                                    </li>
                                                @endif
                                                <li><span>{{ __('Country') . ':' }}</span> {{ $order->shipping_country }}
                                                </li>
                                                <li><span>{{ __('Address') . ':' }}</span> {{ $order->shipping_address }}
                                                </li>
                                            </ul>
                                        </div>
                                    </div>
                                    <div class="col-xxl-4 col-lg-6 col-md-6">
                                        <div class="main-info mb-30">
                                            <h5 class="mb-10">{{ __('Payment Information') }}</h5>
                                            @php
                                            ...
                                            @endphp
                                            <ul class="list">
                                                <li><span>{{ __('Cart Total') . ':' }}(<i
                                                            class="far fa-minus text-success"></i>)
                                                        :</span>{{ $position == 'left' ? $symbol : '' }}{{ number_format($order->total, 2, '.', ',') }}{{ $position == 'right' ? $symbol : '' }}
                                                </li>
                                                <li><span>{{ __('Discount') . ':' }}(<i
                                                            class="far fa-minus text-success"></i>)
                                                ...
                                                <li>
                                                    <span>
                                                        {{ __('Tax') . ':' }}
                                                        <span dir="ltr" class="m-0 ">{{ '(' . $tax->product_tax_amount . '%)' }}</span>
                                                        <span  class="text-danger m-0">(<i class="far fa-plus"></i>)</span>
                                                        :
                                                    </span>
                                                ...
                                                <li>
                                                    ...
                                                    @if ($order->productType == 'digital')
                                                        <span>
                                                            {{ __('Shipping Cost') . ':' }}(<i
                                                                class="far fa-plus text-danger"></i>):
                                                        </span>
                                                ...
                                                @for ($i = 0; $i < $item->quantity; $i++)
                                                    <tr>
                                                        ...
                                                        <td>
                                                            ...
                                                            @else
                                                                <a href="{{ route('shop.product_details', ['slug' => $item->slug]) }}"
                                                                    target="_blank">
                                                                    {{ $item->productTitle }}
                                                                </a>
                                                            @endif
                                                        ...
                                                        <td>
                                                            ...
                                                            <b>{{ __('Quantity') }}:</b> <span>{{ $item->quantity }}</span><br>
                                                        </td>
                                                        <td>
                            ...
                            <div class="edit-account-info">
                                <a href="{{ route('user.order.index') }}"
                                    class="btn btn-lg btn-primary">{{ __('Go Back') }}</a>
                            </div>
                        </div>
                    </div>
                </div>
