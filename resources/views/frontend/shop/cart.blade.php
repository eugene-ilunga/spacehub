...
@php
    $title = $pageHeading->cart_page_title ?? __('Cart');
@endphp
                    ...
                    <div class="col-lg-9">
                        <form action="#">
                                ...
                                <table class="shopping-table table table-borderless">
                                    <thead>
                                        <tr class="table-heading">
                                            <th scope="col" colspan="2" class="first">{{ __('Products') }}</th>
                                        ...
                                        @foreach ($productCart as $key => $c_product)
                                            ...
                                            <tr class="item" id="cart-product-item{{ $key }}">
                                                ...
                                                <td class="product-desc">
                                                    <h6>
                                                        <a class="product-title lc-2 mb-10"
                                                            href="{{ route('shop.product_details', ['slug' => $c_product['slug']]) }}">
                                                            {{ strlen(@$c_product['title']) > 50
                                                                ? mb_substr(@$c_product['title'], 0, 50, 'UTF-8') . '...'
                                                                : @$c_product['title'] }}
                                                        </a>
                                                    </h6>
                                                    <div class="ratings">
                                                ...
                                                <td class="product-price">
                                                    @php
                                                        $total = $product->current_price * $c_product['quantity'];
                                                        $formattedTotal = number_format($total, 2);
                                                    @endphp
                                                    <h6 class="m-0">
                                                        <span dir="ltr">
                                                            {{ $position == 'left' ? $symbol : '' }}<span
                                                                class="per-product-total">{{ $formattedTotal ?? 0.00 }}</span>
                            ...
                            <div class="btn-groups text-end mb-20">
                                <h6>{{ __('Total Quantity') . ' :' }} <span
                                        id="cart_total_qty">{{ $cart_total_qty }}</span>
                                </h6>
                                <h6>{{ __('Total Price') . ' :' }}
                                    <span dir="ltr">
                                        {{ $position == 'left' ? $symbol : '' }}<span
                                            id="cart_total_price">{{ number_format($cart_total_price, 2) }}</span>{{ $position == 'right' ? $symbol : '' }}
                                    </span>
                                </h6>
                            </div>
