@foreach ($quoteRequests as $quoteRequest)
    <div class="modal fade" id="quoteRequestModal-{{ $quoteRequest->id }}" tabindex="-1" role="dialog"
        aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">{{ __('Quote Request Details') }}
                    </h5>
                ...
                <div class="modal-body">
                    ...
                    <p>{{ !empty($quoteRequest->customer_name) ? $quoteRequest->customer_name : '-' }}</p>

                    <label>{{ __('Email') }}</label>
                    <p>{{ !empty($quoteRequest->customer_email) ? $quoteRequest->customer_email : '-' }}</p>
                    @php $informations = json_decode($quoteRequest->information); @endphp
                    ...
                    @if (!is_null($informations))
                        @foreach ($informations as $key => $information)
                            ...
                            @elseif ($information->type == 5)
                                ...
                                @include('vendors.quote-request.show-text')
