                ...
                <div class="modal-body">
                    <div class="alert alert-warning">
                        <span class="text-warning">
                            {{ __("If any feature has crossed its current subscription package's limit") .
                                ', ' .
                        ...
                        <li class="list-group-item ">
                            <div class="d-flex  justify-content-between">
                                ...
                                @if ($current_package->number_of_slider_image_per_space < 999999)
                                    @if (count($totalSliderImage) > 0)
                                        <button class="btn btn-danger btn-sm remove-button" data-toggle="modal"
                                            data-target="#sliderImageModal">{{ __('Remove') }}</button>
                                    @else
                                        <span
                        ...
                        <li class="list-group-item ">
                            <div class="d-flex  justify-content-between">
                                ...
                                @if ($current_package->number_of_service_per_space < 999999)
                                    @if (count($totalServices) > 0)
                                        <button class="btn btn-danger btn-sm remove-button" data-toggle="modal"
                                            data-target="#spaceServiceModal">{{ __('Remove') }}</button>
                                    @else
                                        <span
                        ...
                        <li class="list-group-item ">
                            <div class="d-flex  justify-content-between">
                                ...
                                @if ($current_package->number_of_option_per_service < 999999)
                                    @if (count($totalOptions) > 0)
                                        <button class="btn btn-danger btn-sm remove-button" data-toggle="modal"
                                            data-target="#serviceOptionModal">{{ __('Remove') }}</button>
                                    @else
                                        <span
                        ...
                        <li class="list-group-item ">
                            <div class="d-flex  justify-content-between">
                                ...
                                @if ($current_package->number_of_amenities_per_space < 999999)
                                    @if (count($remainingAmenities) > 0)
                                        <button class="btn btn-danger btn-sm remove-button" data-toggle="modal"
                                            data-target="#spaceAmenityModal">{{ __('Remove') }}</button>
                                    @else
                                        <span
...
<!--  modal for slider images-->
<div class="modal fade" id="sliderImageModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="page-title" id="exampleModalLabel"><span>{{ __('Remove Slider Image From Space') }}</span>
                </h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            ...
            <div class="modal-body">
                <ul class="list-group">
                    @foreach ($totalSliderImage as $id)
                        @php
                            $space = \App\Models\SpaceContent::where('space_id', $id)
                        ...
                        @if ($space)
                            <li class="list-group-item border d-flex justify-content-between align-items-center">
                                <a
                                    href="{{ route('vendor.space_management.space.edit', ['id' => $space->space_id]) }}">
                                    {{ $space->title }}
                                </a>
...
<!--  modal for space service-->
<div class="modal fade" id="spaceServiceModal" tabindex="-1" aria-labelledby="exampleModalLabel"
    aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="page-title" id="exampleModalLabel"><span>{{ __('Remove Service From Space') }}</span></h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
                ...
                <ul class="list-group">
                    @foreach ($totalServices as $id)
                        ...
                        @if ($space)
                            <li class="list-group-item border d-flex justify-content-between align-items-center">
                                <a
                                    href="{{ route('vendor.space_management.service.view_from_space', ['space_id' => $space->space_id]) . '?language=' . $defaultLang->code }}">
                                    {{ $space->title }}
                                </a>
                                <i class="fas fa-paperclip"></i>
                            </li>
...
<!--  modal for options-->
<div class="modal fade" id="serviceOptionModal" tabindex="-1" aria-labelledby="exampleModalLabel"
    aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="page-title" id="exampleModalLabel"><span>{{ __('Remove Option From Service') }}</span>
                </h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            ...
            <div class="modal-body">
                <ul class="list-group">
                    @foreach ($totalOptions as $id)
                        @php
                            $service = \App\Models\SpaceServiceContent::where('space_service_id', $id)
                                ->where('language_id', $defaultLang->id)
                                ->select('space_service_id', 'title')
                                ->first();
                        @endphp
                        ...
                        @if ($service)
                            <li class="list-group-item border d-flex justify-content-between align-items-center">
                                <a
                                    href="{{ route('vendor.space_management.service.edit', ['id' => $service->space_service_id, 'language' => $defaultLang->code]) }}">
                                    {{ $service->title }}
                                </a>
                                <i class="fas fa-paperclip"></i>
                            </li>
...
<!--  modal for amenities-->
<div class="modal fade" id="spaceAmenityModal" tabindex="-1" aria-labelledby="exampleModalLabel"
    aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="page-title" id="exampleModalLabel"><span>{{ __('Remove Amenities From Space') }}</span>
                </h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            ...
            <div class="modal-body">
                <ul class="list-group">
                    @if ($remainingAmenities)
                        @foreach ($remainingAmenities as $item)
                            <li class="list-group-item border d-flex justify-content-between align-items-center">
                                <a
                                    href="{{ route('vendor.space_management.space.edit', ['id' => $item['space_id']]) }}">
                                    {{ $item['title'] }}
                                </a>
                                <i class="fas fa-paperclip"></i>
                            </li>
