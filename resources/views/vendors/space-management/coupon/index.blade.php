...
@section('content')
                ...
                <div class="card-header">
                    <div class="row">
                        ...
                        <div class="col-lg-5 col-12">
                            <form id="filterForm" action="{{ route('vendor.space_management.coupons.index') }}" method="GET" class="row">
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        @if ($coupon->coupon_type == 'fixed')
                                                            {{ $data['currencyInfo']->base_currency_symbol_position == 'left'
                                                                ? $data['currencyInfo']->base_currency_symbol
                                                                : '' }}{{ $coupon->value }}{{ $data['currencyInfo']->base_currency_symbol_position == 'right'
                                                                ? $data['currencyInfo']->base_currency_symbol
                                                                : '' }}
                                                        @else
                                                            {{ $coupon->value . '%' }}
                                                    ...
                                                    <td>
                                                        ...
                                                        <form class="deleteForm d-inline-block"
                                                            action="{{ route('vendor.space_management.coupons.delete', ['id' => $coupon->id]) }}"
                                                            method="post">
                                                            @csrf
                                                            <button type="submit"
    ...
    {{-- create modal --}}
    @include('vendors.space-management.coupon.create')

    {{-- edit modal --}}
    @include('vendors.space-management.coupon.edit')
@endsection

@section('script')
    <script>
        var getCouponDataUrl = "{{ route('vendor.coupon.data') }}";
        var getSpaceUrl = "{{ route('vendor.coupon.get_space') }}";
    </script>
