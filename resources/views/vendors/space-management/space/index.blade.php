...
@section('content')
                ...
                <div class="card-header">
                    <div class="row">
                        ...
                        <div class="col-lg-2">
                            <form action="" method="GET">
                                ...
                                <select name="space_type" class="form-control select2" onchange="this.form.submit()">
                                    ...
                                    <option value="1" @selected(request()->input('space_type') == '1')>
                                        {{ __('Fixed Timeslot Rental') }}
                                    </option>
                                    <option value="2" @selected(request()->input('space_type') == '2')>
                                        {{ __('Hourly Rental') }}</option>
                                    <option value="3" @selected(request()->input('space_type') == '3')>
                                        {{ __('Multi-Day Rental') }}
                                    </option>
                                </select>
                            </form>
                        ...
                        <div class="col-lg-3 mt-2 mt-lg-0">
                            <a href="{{ route('vendor.space_management.space.select_space_type', ['language' => $defaultLang->code]) }}"
                                class="btn btn-primary btn-sm float-right">
                                <i class="fas fa-plus"></i> {{ __('Add Space') }}
                            </a>
                            <button class="btn btn-danger btn-sm float-right mr-2 d-none bulk-delete"
                                data-href="{{ route('vendor.space_management.space.bulk_delete') }}">
                                        ...
                                        <tbody>
                                            @foreach ($spaces as $space)
                                                <tr>
                                                    ...
                                                    <td>{{ strlen(@$space->category->name) > 70
                                                        ? mb_substr(@$space->category->name, 0, 70, 'UTF-8') . '...'
                                                        : @$space->category->name }}
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td>
                                                        @if (!empty($spaces))
                                                            @if (is_null($space->end_date))
                                                                ...
                                                                @endif
                                                            @elseif ($space->end_date < \Carbon\Carbon::now())
                                                                <a href="#" data-toggle="modal"
                                                                    data-target="#createModal"
                                                                    data-space_id="{{ $space->id }}"
                                                                    class="btn btn-primary btn-sm float-left">
                                                            ...
                                                            @elseif ($space->booking_status == 'approved')
                                                                <span
                                                                    class="badge badge-success">{{ __('Active') }}</span>
                                                            @elseif ($space->booking_status == 'pending')
                                                                <span
                                                                    class="badge badge-warning">{{ __('Pending') }}</span>
                                                            @else
                                                                <a href="#" data-toggle="modal"
                                                    ...
                                                    <td>
                                                        @if (!empty($space))
                                                            <a href="{{ route('vendor.space_management.service.view_from_space', ['space_id' => $space->id, 'language' => $defaultLang->code]) }}"
                                                    ...
                                                    <td>
                                                        @if ($space->space_type == 1)
                                                            <a class="btn btn-sm btn-info"
                                                                href="{{ route('vendor.manage_schedule.time_slot.index', ['space_id' => $space->id, 'language' => $defaultLang->code]) }}">{{ __('Manage') }}</a>
                                                        @else
                                                            {{ __('---') }}
                                                        @endif
                                                    </td>
                                                    ...
                                                    <td>
                                                        <h2 class="d-inline-block">
                                                            <span
                                                                class="badge badge-{{ $space->space_status == 1 ? 'success' : 'danger' }}">

                                                                {{ __($space->space_status == 1 ? 'Active' : 'Deactive') }}
                                                            </span>
                                                        </h2>
                                                    </td>
                                                    <td>
                                                        @if ($currentPackage == null)
                                                            <form class="deleteForm d-block"
                                                                action="{{ route('vendor.space_management.space.delete', ['id' => $space->id]) }}"
                                                                method="post">
                                                                @csrf
                                                                <button type="submit"
                                                        ...
                                                        @else
                                                            <div class="dropdown">
                                                                ...
                                                                <div class="dropdown-menu"
                                                                    ...
                                                                    <a href="{{ route('vendor.space_management.space.edit', ['id' => $space->id, 'language' => $defaultLang->code]) }}"
                                                                        class="dropdown-item">
                                                                        {{ __('Edit') }}
                                                                    </a>
                                                                    <a href="{{ route('vendor.manage_weekend.index', ['space_id' => $space->id, 'language' => $defaultLang->code]) }}"
                                                                        class="dropdown-item"
                                                                        data-space_id ="{{ $space->id }}">
                                                                        {{ __('Weekend') }}
                                                                    </a>
    ...
    {{-- create modal --}}
    @include('vendors.space-management.space.feature-charges')
