                                        ...
                                        <tbody>
                                            @foreach ($services as $service)
                                                <tr>
                                                    ...
                                                    <td>{{ $service['spaceService']->price_type ?? '' }}</td>
                                                    @if ($service['spaceService']->has_sub_services === 0)
                                                        <td class="ltr">{{ @$service['spaceService']->price }}</td>
                                                    @else
                                                        <td>{{ '--' }}</td>
                                                    @endif
                                                            ...
                                                            @foreach ($service['subServices'] as $subservice)
                                                                <div>
                                                                    <strong>{{ __('Title') }} </strong>: <span
                                                                        class="ml-2">{{ $subservice->sub_service_title }}</span>
                                                                    <br>
                                                                    <strong>{{ __('Price') }} </strong>: <span dir="ltr"
                                                                        class="ml-2 ">
                                                                        {{ $orderInfo->currency_symbol_position == 'left' ? $orderInfo->currency_symbol : '' }}
                                                                        {{ $subservice->price }}
                                                                        {{ $orderInfo->currency_symbol_position == 'right' ? $orderInfo->currency_symbol : '' }}</span>
                                                                </div>
                                                                <br>
                                                            @endforeach
                                                        </td>
                                                    ...
                                                    @endif
                                                    <td dir="ltr"> {{ $orderInfo->currency_symbol_position == 'left' ? $orderInfo->currency_symbol : '' }}{{ number_format(@$service['global_service_total_price'], 2) }}
